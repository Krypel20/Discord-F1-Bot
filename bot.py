...
def run_discord_bot():
    ...
    async def f1_announcements_task():
        ...
        while True:
            ...
            try:
                ...
                if session_name in ["Pierwszy trening", "Drugi trening", "Trzeci trening", 'Free Practice 1',
                                    'Free Practice 2', 'Free Practice 3', "Trening"]:
                    if 0 < remaining_time <= 15 * 60:
                        await channel.send(
                            f"### <@&1224668671499178005> {next_session.session_name} zacznie się za **{round(remaining_time / 60)} minut**:checkered_flag: (F1)")
                ...
                print(f'\t**BS4 retrieved old race data**')
    ...
    async def f3_announcements_task():
        ...
        while True:
            ...
            try:
                if session_name in ["Trening"]:
                    if 0 < remaining_time <= 15 * 60:
                        await channel.send(
                            f"### <@&1224668671499178005> Polacy będą jeździć za **{round(remaining_time / 60)} minut**:checkered_flag: (F3)")
                        asyncio.create_task(annouce_session_start(next_session, channel, "F3"))
                        cooldown = round(remaining_time) + 4500
                    else:
                        if remaining_time % 60 > 0:
                            cooldown = remaining_time % 60
                ...
                if session_name in ["Kwalifikacje", "Wyścig", "Sprint", "Sprint Qualifying", "Feature"]:
                    if 0 < remaining_time <= 30 * 60:
                        await channel.send(
                            f"### <@&1224668671499178005> Polacy będą ścigać się za **{round(remaining_time / 60)} minut** :checkered_flag: (F3)")
                        asyncio.create_task(annouce_session_start(next_session, channel, "F3"))
                        cooldown = int(remaining_time) + 5400
                    else:
                        if remaining_time % 60 > 0:
    ...
    async def annouce_session_start(session, channel, formula = "F1"):
        while True:
            current_time = datetime.now()
            remaining_time = round(session.time_left(current_time))  # in seconds

            if remaining_time <= 0:
                print(f"annouce_session_start:\n\t{session.session_name} has begun ({formula})")
                embed = session.get_session_embed()
                await channel.send(embed=embed)
                await channel.send(
                    f"## <@&1224668671499178005> :checkered_flag: **{session.session_name}** się rozpoczął :checkered_flag: ({formula})")
                asyncio.create_task(annouce_session_end(session, channel, formula))
                return
            else:
                print(f'annouce_session_start:\n\tWaiting {remaining_time + 1} seconds to annouce {session.session_name}')
                await asyncio.sleep(remaining_time)

    async def annouce_session_end(session, channel, formula="F1"):
        ...
        while True:
            ...
            if current_datetime >= fun_start_time + session.duration:
                print(f"annouce_session_end:\n\t{session.session_name} has ended ({formula}) ")
                result = f1_week.results_urls
                url = result[0][1]
                table = f1_week.get_results_table_from_url(url)
                await channel.send(
                    f"## <@&1224668671499178005> :checkered_flag: **{session.session_name}** się zakończył :checkered_flag: ({formula}) \n # Rezultat: \n```\n{table}\n```")
                return
            else:
                cooldown = remaining_time.total_seconds() + 600
                print(f'annouce_session_end: \n\tWaiting {cooldown} seconds to annouce {session.session_name} end ({formula})')
                await asyncio.sleep(cooldown)

    @client.event
    async def on_ready():
        await client.tree.sync()
        await client.change_presence(activity=discord.activity.Game(name="FORZA FERRARI"),
    ...
    async def f1(ctx):
        try:
            embed = discord.Embed(
                title=f"{f1_week.flag_emoji} {f1_week.name} {f1_week.flag_emoji} ‏ ‎ ‎ ‎ :calendar:{f1_week.week_start} - {f1_week.week_end} ",
                color=0xEF1A2D)
            ...
            for session in f1_week.sessions:
                embed.add_field(name="",
                                value=f"{session.check_session_status(current_datetime)} **{session.session_name}**",
                                inline=True)
    ...
    async def f3(ctx):
        ...
        try:
            embed = discord.Embed(
                title=f"{f3_week.flag_emoji} {f3_week.name} {f3_week.flag_emoji} ‏ ‎ ‎ ‎ :calendar:{f3_week.week_start} - {f3_week.week_end} ",
                color=0xffffff)
